...
function engScoreCookie(inc) {
	var engScrCookie = getCookie("engScore");
	var myDate = new Date();
	myDate.setYear(myDate.getFullYear()+5);

	if (engScrCookie != null) {
		var newScr = parseInt(engScrCookie) + parseInt(inc);
		setCookie('engScore',newScr,'',myDate,'../../index.html','wellsfargoadvisors.com','');
	} else { 
		setCookie('engScore',inc,'',myDate,'../../index.html','wellsfargoadvisors.com','');
	}
}
...
function s_doPlugins(s) {
	...
	*/
	if(s.getQueryParam('intcid')){
		s.eVar22 = s.crossVisitParticipation(s.getQueryParam('intcid'), 's_ev22', '30', '10', '>', '', 1);
	}
...
+"'Saturday'];if(thisy!=s.currentYear){alert('thisy!=s.currentYear');alert('thisy='+thisy+'and s.currentYear='+s.currentYear);return'Data Not Available'}else{;"
...
/*
 * Plugin: Unified Source Tracking v8.5
 */
...
s.Media.trackVars = "events,prop18,prop23,eVar29,eVar32,eVar33,eVar39";
s.Media.trackEvents = "event37,event38,event39,event40,event41,event42";
s.Media.segmentByMilestones = true;
s.Media.trackMilestones = "50";
s.Media.trackUsingContextData = true
s.Media.contextDataMapping = {
	"a.media.name":"eVar32,prop18",
	"a.media.segment":"eVar33",
	"a.contentType":"eVar39",
	"a.media.timePlayed":"event38",
	"a.media.view":"event39",
	"a.media.segmentView":"event41",
...
s.Media.monitor = function (s,media) {
	if(media.event=="OPEN") {
		s.events='';
		s.prop23="Video View";
		s.eVar29=engScores['video view'];
		engScoreCookie(parseInt(engScores['video view']));
		s.addEvent("event37="+parseInt(engScores['video view']));
		s.Media.track(media.name);
	}
}

function mediaPlayFirstTime(vidName,vidDur,vidPlayer,vidPos) {
	s.Media.open(vidName,vidDur,vidPlayer);
	s.Media.play(vidName,vidPos);
}

function mediaPlay(vidName,vidPos) {
	s.Media.play(vidName,vidPos);
}

function mediaPause(vidName,vidPos) {
	s.Media.stop(vidName,vidPos);
}

function mediaSeeking(vidName,vidPos) {
	s.Media.stop(vidName,vidPos);
}

function mediaSeeked(vidName,vidPos) {
	s.Media.play(vidName,vidPos);
}

function mediaEnd(vidName,vidPos) {
	s.Media.stop(vidName,vidPos);
